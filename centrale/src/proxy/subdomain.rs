...
async fn normal_subdomain() {
    use crate::request::{
        _create_wildcard_request_with_host, _user_create_request, get_centrale_cookie,
    };
    use crate::user::register::_create_test_pool;
...
async fn domain_with_two_subdomains_fails() {
    ...
    // MAKE WILDCARD REQUEST WITH COOKIE
    let wild_req =
        _create_wildcard_request_with_host(cookie, "https://hello.hello.hello.ee".to_string());
    let auth_resp = test::call_service(&app, wild_req).await;
...
async fn just_domain_without_wildcard_fails() {
    ...
    let cookie = format!("centrale={}", cookie_value);
    // MAKE WILDCARD REQUEST WITH COOKIE
    let wild_req = _create_wildcard_request_with_host(cookie, "https://hello.ee".to_string());
    let auth_resp = test::call_service(&app, wild_req).await;
    //
    assert!(auth_resp.status().is_client_error());
}
